...
def speed_heterogeneity_linear_model( trajectory_dict, speeds_dict):

    from sklearn.linear_model import LinearRegression

    n_permute = 1000

    ###Create a set of predictor variables for the different samples; 
    experiment= 'fish_T'
    treatment = 'control'
    samples = list(trajectory_dict[experiment][treatment].keys())
    indicator_vars = []
    speeds_list = []
    for i in range(len(samples)):
        sample = samples[i]
        indicator_var = [0 for l in range(len(samples))]
        indicator_var[i] = 1
        base_points = []
        speeds_within = []
    #print(indicator_var)
        for traj_ind in trajectory_dict[experiment][treatment][sample]:
            #print(traj_ind)
            if traj_ind in speeds_dict[experiment][treatment][sample]:
                #print(traj_ind)
                speed = numpy.mean(speeds_dict[experiment][treatment][sample][traj_ind])

                speeds_list.append(speed)
                indicator_vars.append(indicator_var)

                base_point = trajectory_dict[experiment][treatment][sample][traj_ind][0][1:]

                speeds_within.append(speed)
                base_points.append(base_point)
        speeds_within = numpy.array(speeds_within)
        base_points = numpy.array(base_points)

        #spatial_reg = LinearRegression().fit(base_points,speeds_within)
        #spatial_reg_r2 = spatial_reg.score(base_points,speeds_within)
        #fig,(ax1,ax2) = pt.subplots(1,2,figsize=(9,4))
        #ax1.scatter(base_points[:,0],speeds_within)
        #ax2.scatter(base_points[:,1],speeds_within)
        #pt.show()
        #spatial_r2_perm = []
        #for n in range(n_permute):
        #    bp_permute = numpy.random.permutation(base_points)
        #    ft = LinearRegression().fit(bp_permute,speeds_within)
        #   spatial_r2_perm.append(ft.score(bp_permute,speeds_within))
        #spatial_r2_perm = numpy.array(spatial_r2_perm)
        #pval = numpy.sum(spatial_r2_perm > spatial_reg_r2)/n_permute ###Fraction of the time that the null model R^2 exceeds the actual
        #print(sample,len(speeds_within),spatial_reg_r2,spatial_reg.coef_,'p=',pval)

    
    indicator_vars = numpy.array(indicator_vars)
    speeds_list = numpy.array(speeds_list)
    print(indicator_vars.shape)
    print(indicator_vars[0:10,:])
    print(speeds_list[0:10])
    print(indicator_vars[200:210,:])
    print(speeds_list[200:210])
    

    reg = LinearRegression(fit_intercept=False).fit(indicator_vars,speeds_list)
    ypred = reg.predict(indicator_vars)
    print(numpy.var(ypred)/numpy.var(speeds_list))
    pt.figure()
    pt.scatter(ypred,speeds_list,s=4,alpha=.4)
    pt.xlabel('Sample ID predictor')
    pt.ylabel(r'Trajectory speed ($\mu$m/min)')
    pt.savefig('/Users/ejerison/Dropbox/imaging_data/figures/revision_figures/speeds_by_sample.pdf')
    reg_score = reg.score(indicator_vars,speeds_list)
    print('sanity check, ',reg.get_params())
    print('sanity check, ',reg.coef_)
    print('sanity check, ',reg.intercept_)
    r2_perm = []
    for n in range(n_permute):
        ind_permute = numpy.random.permutation(indicator_vars)
        ft = LinearRegression().fit(ind_permute,speeds_list) ###Note that numpy.random.permutation shuffles along the first index for a multi-d array
        r2_perm.append(ft.score(ind_permute,speeds_list))
    reg_score = reg.score(indicator_vars,speeds_list)
    r2_perm = numpy.array(r2_perm)
    
    print('Fraction of variance due to sample, ', reg_score)
    
    #ax.hist(r2_perm,alpha=.6)
    #ax.axvline(reg_score,0,1,color='k')
    pval = numpy.sum(r2_perm > reg_score)/n_permute
    print('sample to sample variation, p=',pval)
...
def speed_dist_angle_dist_KS_test( speeds_dict, speed_angle_coupling_dict ):
    ...
    print('Overall Speed distribution nsteps, ',len(all_speeds))
    print('Overall Angle distribution nsteps, ',len(all_angles))
...
def turn_angles_by_speed_class( step_angle_coupling_dict, speeds_dict, selectable_inds_by_speed_class, ax, source_data ):
    ...
    file = open(source_data,'w')
    file.write('Angle bins (rad)'+'\t' + (',').join([str(a) for a in angle_bins])+ '\n')
    for speed_class in range(5):
        
        bs_data = numpy.array(angle_dists_bootstrapped[speed_class])
        
        medians = numpy.percentile(bs_data,50,axis=0)
        lb = numpy.percentile(bs_data,2.5,axis=0)
        ub = numpy.percentile(bs_data,97.5,axis=0)
        
        ax.bar(angle_bins[:-1] + speed_class*offset, medians, yerr = [medians - lb, ub - medians], width = .9*offset)
        file.write('Probability density (median)' + '\t' + 'Speed class '+ str(speed_class) + '\t' + (',').join([str(s) for s in medians]) + '\n')
        file.write('Probability density (2.5 percentile)' + '\t' + 'Speed class '+ str(speed_class) + '\t' + (',').join([str(s) for s in lb]) + '\n')
        file.write('Probability density (97.5 percentile)' + '\t' + 'Speed class '+ str(speed_class) + '\t' + (',').join([str(s) for s in ub]) + '\n')
...
def speed_angle_coupling_by_speed_class( step_angle_coupling_dict, speeds_dict, interval, selectable_inds_by_speed_class, ax, source_data ):
    ...
    for speed_class in range(5):
        ...
        lb = numpy.percentile(angle_stat_bootstrapped[speed_class],2.5,axis=0)
        ub = numpy.percentile(angle_stat_bootstrapped[speed_class],97.5,axis=0)
        ...
        file.write('Local speed (um/min)'+'\t'+'Speed Class '+str(speed_class) + '\t'+ (',').join([str(x) for x in median_xlocs])+'\n')
        file.write('Cosine of turn angles (median)'+'\t'+'Speed Class '+str(speed_class) + '\t'+ (',').join([str(x) for x in medians])+'\n')
        file.write('Cosine of turn angles (2.5 percentile)'+'\t'+'Speed Class '+str(speed_class) + '\t'+ (',').join([str(x) for x in lb])+'\n')
        file.write('Cosine of turn angles (97.5 percentile)'+'\t'+'Speed Class '+str(speed_class) + '\t'+ (',').join([str(x) for x in ub]))
...
def boutx_distribution( speeds_dict,trajectory_dict_polar_interp,ax,source_data):
    n_tsteps_min = 30
    n_bootstrap = 200
    ...
    cf,res = curve_fit(log_exp, xlocs, numpy.log(histvals))
    print(cf)
    hist_stat_bs = numpy.array(hist_stat_bs)
    lb = numpy.percentile(hist_stat_bs,2.5,axis=0)
    ub = numpy.percentile(hist_stat_bs,97.5,axis=0)
    ax.set_xscale('log')
    ax.set_yscale('log')
    ...
    file = open(source_data,'w')
    file.write('Bout sizes (normalized)' + '\t' + (',').join([str(x) for x in xlocs]) + '\n')
    file.write('Probability density' + '\t' + (',').join([str(x) for x in histvals]))
...
def all_cells_manifold_3d(step_angle_coupling_dict, speeds_dict, ax, source_data ):
        ...
        if 'fish' in experiment:
            for treatment in mean_speeds[experiment]:
                if 'control' in treatment and 'highfreq' not in treatment:
                    ...
                    file.write('Speeds (um/min)' + '\t'+ (',').join([str(s) for s in x])+'\n')
                    file.write('Average of cosine of turn angles' + '\t'+ (',').join([str(s) for s in y])+'\n')
                    file.write('Additional turn angle-speed correlation' + '\t'+ (',').join([str(s) for s in z]))
                    #cbar = pt.colorbar(sct,ax=ax)
                    #my_cmap = matplotlib.cm.get_cmap(cmap='RdBu',vmin=-1*upper,vmax=upper)
                    #my_norm = matplotlib.colors.Normalize(vmin=lower, vmax=upper, clip=False)
                    #my_mappable = matplotlib.cm.ScalarMappable(cmap='viridis',norm=my_norm)
...
def persistence_time_binned_fishonly(corr_times_dict, speeds_dict, ax, source_data):
    ...
    lb = numpy.percentile(corr_times_stat_bs,2.5,axis=0)
    ub = numpy.percentile(corr_times_stat_bs,97.5,axis=0)
    ...
    file.write('Speeds (um/min)'+'\t'+(',').join([str(s) for s in xlocs])+'\n')
    file.write('Correlation times (median)'+'\t'+(',').join([str(s) for s in corr_times_med])+'\n')
    file.write('Correlation times (2.5 percentile)'+'\t'+(',').join([str(s) for s in lb])+'\n')
    file.write('Correlation times (97.5 percentile)'+'\t'+(',').join([str(s) for s in ub]))
    file.close()

def persistence_time_binned_shallowangle(corr_times_dict, speeds_dict, ax):
    ...
    ax.legend([handle1,handle2],['SPC','UPT'],loc="upper left")
    ax.set_xlabel('Cell speed ($\mu$m/min)')
    ax.set_ylabel(r'Persistence time (min), within $\pi$/6')

def PSDs_overall(corr_times_dict, trajectory_dict_polar, ax, source_data):
    ...
    for experiment in experiment_list:
        ...
        for treatment in corr_times_dict[experiment]:
            if 'control' in treatment and 'highfreq' not in treatment:
                ...
                file.write('frequencies'+'\t'+(',').join((str(h) for h in hz_frequencies[1:]*60))+'\n')
                file.write('PSD' + '\t' + (',').join([str(p) for p in psd_overall[1:]]))
                file.close()
...
def MSDs_by_speed_class_fishonly(speeds_dict, step_angle_coupling_dict, drift_corrected_traj_dict, selectable_inds_by_speed_class, ax, source_data):
    ...
    file = open(source_data,'w')
    file.write('tau (min)' + '\t' + (',').join([str(t) for t in numpy.array(taus)/60]) + '\n')
    n_bootstrap = 200
    speed_label_list = []
    for speed_class in range(5):
        ...
        for n in range(n_bootstrap):
            ...
            for tau in taus:
                ...
                msd_mean = msd/interval_counter
                
                mean_msd_of_tau.append(msd_mean)
        ...
        lb = numpy.percentile(msds_bs,2.5,axis=0)
        ub = numpy.percentile(msds_bs,97.5,axis=0)
        ...
        file.write('MSD, scaled (median)' + '\t' + 'Speed class '+ str(speed_class) + (',').join([str(m) for m in msd_med_scaled]) + '\n')
        file.write('MSD, scaled (2.5 percentile)' + '\t' + 'Speed class '+ str(speed_class) + (',').join([str(m) for m in lb_scaled]) + '\n')
        file.write('MSD, scaled (97.5 percentile)' + '\t' + 'Speed class '+ str(speed_class) + (',').join([str(m) for m in ub_scaled]) + '\n')
...
def speed_rank_corr(trajectory_dict_polar, speeds_dict, ax, source_data):
    ...
    corr_vals_95 = numpy.percentile(corr_vals_bs,97.5,axis=0)
    corr_vals_5 = numpy.percentile(corr_vals_bs,2.5,axis=0)
    ...
    null_vals_95 = numpy.percentile(null_vals_bs,97.5,axis=0)
    null_vals_5 = numpy.percentile(null_vals_bs,2.5,axis=0)
    ...
    file = open(source_data,'w')
    file.write('Time lag' + '\t' + (',').join((str(t) for t in (time_chunks4[:-1]+45)/60.)) + '\n')
    file.write('Correlation (median)' + '\t' + (',').join((str(t) for t in corr_vals_50)) + '\n')
    file.write('Correlation (2.5 percentile)' + '\t' + (',').join((str(t) for t in corr_vals_5)) + '\n')
    file.write('Correlation (97.5 percentile)' + '\t' + (',').join((str(t) for t in corr_vals_95)) + '\n')
    file.write('Correlation (median), null model' + '\t' + (',').join((str(t) for t in null_vals_50)) + '\n')
    file.write('Correlation (2.5 percentile), null model' + '\t' + (',').join((str(t) for t in null_vals_5)) + '\n')
    file.write('Correlation (97.5 percentile), null model' + '\t' + (',').join((str(t) for t in null_vals_95)))
    file.close()
...
def MSD_dists_by_speed_class(speeds_dict, drift_corrected_traj_dict, tau, ax):
    ...
    for tau in taus:
        for speed_class in range(5):
            ...
            lb = numpy.percentile(msd_stat_bs,2.5,axis=0)
            ub = numpy.percentile(msd_stat_bs,97.5,axis=0)
...
def MSD_v_speed(speeds_dict, corr_times_dict, drift_corrected_traj_dict, ax, source_data):
    ...
    taus = [9*60,1080,1080+9*60,1080+18*60,1080+27*60]
    ...
    for tau in taus:
        ...
        file.write('speeds' + '\t' + 'tau='+str(int(tau/60)) + '\t' + (',').join((str(x) for x in xlocs)) + '\n')
        file.write('D (median)' + '\t' + 'tau='+str(int(tau/60)) + (',').join((str(x) for x in msd_stat_med_sc)) + '\n')
        file.write('D (2.5 percentile)' + '\t' + 'tau='+str(int(tau/60)) + (',').join((str(x) for x in lb_sc)) + '\n')
        file.write('D (97.5 percentile)' + '\t' + 'tau='+str(int(tau/60)) + (',').join((str(x) for x in ub_sc)) + '\n')
    
    legend1=ax.legend(tau_list,fontsize=8,loc="upper left")
    
    
    print(len(xlocs_all),len(msd_stat_all))
    popt_spc,pcov_spc = curve_fit( make_spc(*P_fit), xlocs_all,numpy.log10(msd_stat_all), sigma = numpy.log10(numpy.array(ub_all)/numpy.array(lb_all)),bounds=(0,numpy.inf) )
    popt_prw,pcov_prw = curve_fit( prw, xlocs_all,numpy.log10(msd_stat_all), sigma = numpy.log10(numpy.array(ub_all)/numpy.array(lb_all)), bounds=(0,numpy.inf) )
    
    my_spc = make_spc(*P_fit)
    
    popt_spc_free,pcov_spc_free = curve_fit( spc, xlocs_all,numpy.log10(msd_stat_all), sigma = numpy.log10(numpy.array(ub_all)/numpy.array(lb_all)), p0=[.2,.3],bounds=(0,numpy.inf) )
...
def MSD_overall(speeds_dict, drift_corrected_traj_dict, ax, inset_ax, source_data):
    ...
    file = open(source_data,'w')
    file.write('tau (min)' + '\t' + (',').join(str(t) for t in taus)+'\n')
    file.write('MSD median (square microns)' + '\t' + (',').join(str(m) for m in med_stat))
    file.write('MSD 2.5 percentile' + '\t' + (',').join(str(l) for l in lb)+'\n')
    file.write('MSD 97.5 percentile' + '\t' + (',').join(str(l) for l in ub))
    file.close()
...
def MSD_overall_shorter(speeds_dict, drift_corrected_traj_dict, ax, inset_ax):
    ...
    taus = [45,90,135,180,225,270,315,360,450,540,630,720,810,900]#,1080+180*2,1080+180*3,1080+180*4,1080+180*6,1080+180*8,1080+180*10,1080+180*12,1080+180*14,1080+180*18,1080+180*22]
...
def speed_angle_heterogeneity_panels_revised(speeds_dict, trajectory_dict_polar, speed_angle_coupling_dict, axis_lists,source_data_filename1, source_data_filename2, source_data_filename3):
    ...
    file = open(source_data_filename1,'w')
                        ...
                        for traj_ind in trajectory_dict_polar[experiment][treatment][sample]:
                            ...
                            if traj_ind in speed_angle_coupling_dict[experiment][treatment][sample]:
                                ...
                                if sample == focal_sample and traj_ind in chosen_indexes:        
                                    ...
                                    file.write('Time (min)' + '\t' + 'trajectory '+ str(traj_ind) + '\t'+ (',').join((str(t) for t in individual_trajectories[traj_ind]['times'])) + '\n')
                                    file.write('vx' + '\t'  + str(traj_ind) + '\t'+ (',').join((str(t) for t in individual_trajectories[traj_ind]['vx'])) + '\n')           
                            elif sample == focal_sample and traj_ind in chosen_indexes:
                                print(traj_ind,'why')
    file.close()
    
    files = open(source_data_filename2,'w')
    filea = open(source_data_filename3,'w')
    ...
    shist,binedges=numpy.histogram(all_speeds,bins=speed_bins,density=True)
    ahist,binedges=numpy.histogram(all_angles,bins=angle_bins,density=True)
    files.write('speed bins' + '\t' + (',').join(str(s) for s in speed_bins)+'\n')
    filea.write('angle bins' + '\t' + (',').join(str(a) for a in angle_bins)+'\n')
    files.write('overall speed dist' + '\t' + (',').join(str(s) for s in shist) + '\n')
    filea.write('overall angle dist' + '\t' + (',').join(str(a) for a in ahist) + '\n')                                           
    for traj_ind in chosen_indexes:
        ...
        panel_ind +=1    
        print(traj_ind,len(individual_trajectories[traj_ind]['speeds']))
        shist,binedges=numpy.histogram(individual_trajectories[traj_ind]['speeds'],bins=speed_bins,density=True)
        ahist,binedges=numpy.histogram(individual_trajectories[traj_ind]['angles'],bins=angle_bins,density=True)
        files.write('speed dist' + '\t' + 'trajectory '+str(traj_ind) +'\t' + (',').join(str(s) for s in shist) + '\n')
        filea.write('angle dist' + '\t' + 'trajectory '+str(traj_ind) +'\t' + (',').join(str(a) for a in ahist) + '\n')
    files.close()
    filea.close()                                           
...
def angle_speed_correlations_with_perturbations(speeds_dict,relative_angle_dict,experiment,ax,ax2):
    ...
    for treatment in angles_subset:
        ...
        lb = numpy.percentile(ct_stat_bs,2.5,axis=0)
        ub = numpy.percentile(ct_stat_bs,97.5,axis=0)
...
def angle_speed_correlations_with_perturbations_all_cells(speeds_dict,relative_angle_dict,experiment,ax,ax2,source_data1,source_data2):
    ...
    for treatment in angles_subset:
        if 'highfreq' not in treatment:
            ...
            print('Calculating binned statistic; expected to report nan for unused bins')
            xlocs,bins,nbins = binned_statistic(speeds_subset[treatment],speeds_subset[treatment],bins=speed_bins)
    ...
    for treatment in angles_subset:
        if 'highfreq' not in treatment:
            ...
            file.write(experiment + '\t' + treatment + '\t' + 'speeds' + '\t' + (',').join((str(x) for x in speeds_subset[treatment])) + '\n')
            file.write(experiment + '\t' + treatment + '\t' + 'turn angle cosines' + '\t' + (',').join((str(a) for a in angles_subset[treatment])) + '\n')

    ax.legend(treatment_label_list)
    
    file.close()
                                               
    file = open(source_data2,'w')
    speed_bins = numpy.arange(0,18.1,1)
    file.write('speed bins (um/min)' + '\t' + (',').join([str(s) for s in speed_bins]) + '\n')                                           
    ...
    for treatment in treatment_list:
        ...
        sns.distplot(speeds_subset[treatment],ax=ax2,kde=False,bins=speed_bins,norm_hist=True,color=treatment_color_dict[treatment],hist_kws={"histtype": "step", "linewidth": 2,"alpha": 1, "color": treatment_color_dict[treatment]})
        hist,binedges = numpy.histogram(speeds_subset[treatment],bins=speed_bins,density=True)
        file.write('histogram values' + '\t' + experiment + '\t' + treatment + '\t' + (',').join((str(h) for h in hist)) + '\n')                                       
    ax.set_xlabel('Cell speed ($\mu$m/min)')
