...
####This script is a top-level wrapper for paper figures. It first imports the trajectory data that is used in most figure panels; these data structures are passed to subfunctions in the figure_panel_functions_09192019.py library for plotting.
####This script takes axes returned by figure_panel_functions_09192019 and arranges them into multiplanel figures.
...
def plot_figure2(master_trajectory_dict, speeds_dict, step_angle_coupling_dict, trajectory_dict_polar_interp):
	...
	speed_class_inds = figure_panel_functions_09192019.define_speed_classes( speeds_dict, step_angle_coupling_dict, master_trajectory_dict)
	
	####Make each of the panel plots
	
	figure_panel_functions_09192019.turn_angles_by_speed_class( step_angle_coupling_dict, speeds_dict,speed_class_inds, ax = my_axes[0])
	print('finished with turn angle dist')
	figure_panel_functions_09192019.speed_angle_coupling_by_speed_class( step_angle_coupling_dict, speeds_dict, 45, speed_class_inds, ax = my_axes[1])
	print('finished with speed-angle coupling')
	figure_panel_functions_09192019.persistence_time_binned_fishonly( corr_times_dict, speeds_dict, ax = my_axes[2] )
	print('finished with persistence time')
	figure_panel_functions_09192019.MSDs_by_speed_class_fishonly(speeds_dict, step_angle_coupling_dict, master_trajectory_dict, speed_class_inds, ax = my_axes[3] )
	print('finished with MSDs')
	figure_panel_functions_09192019.MSD_v_speed(speeds_dict, corr_times_dict, master_trajectory_dict, ax = my_axes[4])
	print('finished with MSDs vs speed1')
	figure_panel_functions_09192019.MSD_v_speed_deviations(speeds_dict, corr_times_dict, master_trajectory_dict, ax = my_axes[5])
	...
	#figure_panel_functions_09192019.PSDs_by_speed_class(
...
def plot_figure2_subsampled(master_trajectory_dict, speeds_dict, step_angle_coupling_dict, trajectory_dict_polar_interp):
	...
	speed_class_inds = figure_panel_functions_09192019.define_speed_classes_subsamp( speeds_dict, step_angle_coupling_dict, master_trajectory_dict)
	
	####Make each of the panel plots
	
	figure_panel_functions_09192019.turn_angles_by_speed_class( step_angle_coupling_dict, speeds_dict,speed_class_inds, ax = my_axes[0])
	print('finished with turn angle dist')
	figure_panel_functions_09192019.speed_angle_coupling_by_speed_class( step_angle_coupling_dict, speeds_dict, 90, speed_class_inds, ax = my_axes[1])
	print('finished with speed-angle coupling')
	figure_panel_functions_09192019.persistence_time_binned_fishonly( corr_times_dict, speeds_dict, ax = my_axes[2] )
	print('finished with persistence time')
	figure_panel_functions_09192019.MSDs_by_speed_class_fishonly_sub(speeds_dict, step_angle_coupling_dict, master_trajectory_dict, speed_class_inds, ax = my_axes[3] )
	print('finished with MSDs')
	figure_panel_functions_09192019.MSD_v_speed(speeds_dict, corr_times_dict, master_trajectory_dict, ax = my_axes[4])
	print('finished with MSDs vs speed1')
	figure_panel_functions_09192019.MSD_v_speed_deviations(speeds_dict, corr_times_dict, master_trajectory_dict, ax = my_axes[5])
...
def plot_figure1(master_trajectory_dict, trajectory_dict_polar_interp, step_angle_coupling_dict):
	...
	figure_panel_functions_09192019.MSD_overall(speeds_dict, master_trajectory_dict, ax=my_axes[0],inset_ax=my_axes[1])
	print('Done calculating overall MSDs')
	figure_panel_functions_09192019.PSDs_overall(corr_times_dict, trajectory_dict_polar_interp, ax=my_axes[2])
	print('Done calculating overall PSDs')
	figure_panel_functions_09192019.boutx_distribution( speeds_dict,trajectory_dict_polar_interp,ax=my_axes[3])
	print('Done calculating boutx')
	figure_panel_functions_09192019.speed_angle_heterogeneity_panels_revised(speeds_dict, trajectory_dict_polar, step_angle_coupling_dict, axis_lists=[[my_axes[4],my_axes[5],my_axes[6]],[my_axes[7],my_axes[8],my_axes[9]],[my_axes[10],my_axes[11],my_axes[12]],[my_axes[13],my_axes[14],my_axes[15]]])
...
def plot_figure4_mouse_dicty(speeds_dict, corr_times_dict):
	...
	figure_panel_functions_09192019.angle_speed_correlations_with_perturbations_all_cells( speeds_dict, step_angle_coupling_dict, experiment='Gerard_mouse_T', ax = my_axes[0], ax2 = my_axes[2])
	figure_panel_functions_09192019.angle_speed_correlations_with_perturbations_all_cells( speeds_dict, step_angle_coupling_dict, experiment='Gautreau_dicty', ax = my_axes[1], ax2 = my_axes[3])
...
def plot_figure3_fish(speeds_dict, corr_times_dict):
	...
	figure_panel_functions_09192019.angle_speed_correlations_with_perturbations_all_cells( speeds_dict, step_angle_coupling_dict, experiment='fish_T', ax = my_axes[0], ax2 = my_axes[1])
...
def plot_anova(step_angle_coupling_dict, speeds_dict):
	...
	figure_panel_functions_09192019.manifold_anova(step_angle_coupling_dict, speeds_dict, ax1, ax2 )
	panel_letters = ['A','B','C','D','E','F','G','H','I']
	my_axes = [ax1,ax2]
	for i in range(len(my_axes)):
...
def plot_figure3_pairedonly(speeds_dict, corr_times_dict):
	...
	figure_panel_functions_09192019.angle_speed_correlations_with_perturbations_paired_samples( speeds_dict, step_angle_coupling_dict, experiment='fish_T', ax = my_axes[0], ax2 = my_axes[1])
